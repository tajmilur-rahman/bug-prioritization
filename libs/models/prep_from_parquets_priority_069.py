...
def main():
    ...
    print(f"{fusion_mode} - {label_kind}")
    ...
    # For severity, train with S2-S4
    if label_col.startswith("severity"):
        s1_label = ~df[label_col].str.lower().isin(["s2", "s3", "s4"])
        keep_mask = ~(miss_embed | miss_topic | miss_label | s1_label)
    else:
    ...
    E_te = stack_embed(test_df) if len(test_df) > 0 else None
    ...
    TP_te = np.round(p_text.transform(E_te), 6).astype("float32") if E_te is not None else None
    ...
    Tcen_te = topic_centroid_block(test_df) if len(test_df) > 0 else None
    ...
    T_emb_te = p_topic.transform(Tcen_te).astype("float32") if Tcen_te is not None else None
    ...
    Xn_te = numeric_block(test_df) if len(test_df) > 0 else None
    ...
    Xcat_te = apply_cat(test_df, cat_tables, dim=16) if len(test_df) > 0 else None
    ...
    Xa_te = actors_block(test_df, actors_cfg) if len(test_df) > 0 else None
    ...
    def build_parts(tp, tpc, xc, xa, xn, sim):
        ...
        if fusion_mode.startswith("C"):
            if fusion_mode == "C1":
                return [tpc, xc, xn, sim]
            elif fusion_mode == "C2":
                return [xc, xn, sim]
            else:
                raise ValueError(f"Unknown fusion: {fusion_mode}")
    ...
    X_te = hstack(build_parts(TP_te, T_emb_te, Xcat_te, Xa_te, Xn_te, S_te)) if len(test_df) > 0 else []
    ...
    X_te = scaler.transform(X_te).astype("float32") if len(test_df) > 0 else []

    # labels
    y_tr = train_df[label_col].astype(str).to_numpy()
    y_va = val_df[label_col].astype(str).to_numpy()
    y_te = test_df[label_col].astype(str).to_numpy() if len(test_df) > 0 else []
    ...
    # save bug ids of train/val/test sets for later matching
    print(f"[prep] Save ids of Splits → train={len(train_df)}, val={len(val_df)}, test={len(test_df)}")
    np.save(out_dir / 'train_ids.npy', train_df["id"].values)
    np.save(out_dir / 'val_ids.npy',   val_df["id"].values)
    np.save(out_dir / 'test_ids.npy',  test_df["id"].values)
    ...
    if fusion_mode == "A1":
        ...
        used_blocks = ["topic_pca", "categorical", "numeric", "topK_sims"]
    elif fusion_mode == "C2":
        used_blocks = ["categorical", "numeric", "topK_sims"]
