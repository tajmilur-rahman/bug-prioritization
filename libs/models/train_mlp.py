...
def main():
    ...
    if os.getenv("IMBALANCE", "weighted_ce") == "logit_adjust":
        ...
        wmin = float(os.getenv("CLASS_WEIGHT_CLIP_MIN", "1.0"))
        wmax = float(os.getenv("CLASS_WEIGHT_CLIP_MAX", "7.0"))
        weights = torch.ones(num_classes, dtype=torch.float32)
        for c, w in cw.items():
            if c < num_classes:
                v = max(wmin, min(float(w), wmax))
                weights[c] = v
